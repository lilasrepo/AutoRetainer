...
public unsafe class CabinetManager : IDisposable
{
    ...
    {
        Svc.AddonLifecycle.RegisterListener(AddonEvent.PreFinalize /* porting-note(api12): PreClose is API15-only; PreFinalize fires when the cabinet addon is dismissed on 7.1 */, ["Cabinet", "CabinetWithdraw"], OnCabinetClose);
    }
    ...
    public uint[] CabinetValidItems => field ??= Svc.Data.GetExcelSheet<Cabinet>()!.Where(x => x.Item.RowId != 0).Select(x => x.Item.RowId).ToArray();
    ...
    public bool TryGetStoredCabinetItems(out bool cached, out List<uint> items)
    {
        ...
        {
            foreach(var x in Svc.Data.GetExcelSheet<Cabinet>()!)
            {
                if(x.Item.RowId != 0)
                {
                    if(UIState.Instance()->Cabinet.IsItemInCabinet((int)x.RowId)) // porting-note(api12): 7.1 FCS IsItemInCabinet takes int, not uint
                    {
                ...
                for(var i = 0; i < inv->GetSize(); i++)
                {
                    ...
                    if(item->GetConditionPercentage() == 100 && !Utils.IsProtected(item->ItemId))
                    {
                        ...
                        if(CanItemGoIntoCabinet(id) && !storedItems.Contains(id) && !Utils.IsProtected(item->ItemId))
                        {
                            ...
                            {
                                var cabItem = Svc.Data.GetExcelSheet<Cabinet>()!.FirstOrNull(x => x.Item.RowId == id);
                                if(cabItem != null && cabItem.Value.RowId != 0 && !UIState.Instance()->Cabinet.IsItemInCabinet((int)cabItem.Value.RowId) && TryGetAddonByName<AtkUnitBase>("Cabinet", out var addon) && addon->IsReady())
                                {
                                    // porting-note(api12/B1): FFXIVClientStructs 7.1 Cabinet struct lacks StoreCabinetItem
                                    // (game-7.5 addition). Neutered until the 7.1 Armoire-store path is verified in-client; the
                                    // feature is default-OFF (InventoryManagementSettings.EnableCabinetAutoDelivery).
                                    // TODO(api12): wire the real 7.1 Armoire store call once confirmed on the TC client.
                                    var result = false;
                                    PluginLog.Debug($"Store cabinet item {ExcelItemHelper.GetName(cabItem.Value.Item.RowId, true)}, result={result} (api12 stub — store no-op)");
    ...
    public void Dispose()
    {
        Svc.AddonLifecycle.UnregisterListener(AddonEvent.PreFinalize /* porting-note(api12): PreClose is API15-only; PreFinalize fires when the cabinet addon is dismissed on 7.1 */, ["Cabinet", "CabinetWithdraw"], OnCabinetClose);
    }
}
